...
function TechTag({ children }) {
  return (
    <span className="px-2.5 py-1 rounded-lg text-xs font-medium bg-graphite text-bone/70">
      {children}
    </span>
  );
}
...
function Role({ role, isCurrent, isEnglish }) {
  ...
  return (
    <div className="relative pl-8 tablet:pl-10 pb-10 last:pb-0">
      {/* Rail */}
      <div className="absolute left-[5px] top-3 bottom-0 w-px bg-bone/[0.08]" />

      {/* Node */}
      <div
        className={`absolute left-0 top-[7px] w-[11px] h-[11px] rounded-full border-2 ${
          isCurrent
            ? 'bg-ember border-ember shadow-[0_0_0_4px_rgba(147,51,234,0.15)]'
            : 'bg-ink border-bone/20'
        }`}
      />

      <h4 className="font-display text-lg tablet:text-xl font-bold text-bone leading-snug tracking-[-0.015em]">
        {role.title}
      </h4>

      <p className="mt-1 text-xs font-semibold text-ember tabular-nums">{period}</p>

      <ul className="mt-4 space-y-2.5">
        {role.highlights.map((highlight) => (
          <li
            key={highlight}
            className="relative pl-4 text-sm text-bone/55 leading-relaxed text-pretty before:absolute before:left-0 before:top-[0.6em] before:w-1 before:h-1 before:rounded-full before:bg-ember/50"
          >
            {highlight}
          </li>
        ))}
      ...
      {career.map((company, companyIndex) => (
        <article data-reveal
          ...
          <div className="laptop:sticky laptop:top-28 laptop:self-start">
            ...
            <h3 className="font-display text-xl tablet:text-2xl font-bold text-bone leading-tight tracking-[-0.02em]">
              {company.company}
            </h3>
            ...
            {company.duration && (
              <p className="mt-1 text-xs text-ash/70">{company.duration}</p>
            )}
          </div>
