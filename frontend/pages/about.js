  ...
  return (
    ...
    >
      <div>
        ...
        <motion.h1
          variants={fadeUp}
          className="font-display text-4xl tablet:text-5xl laptop:text-6xl desktop:text-7xl font-bold text-gray-900 dark:text-white leading-[0.98] tracking-[-0.04em] text-balance"
        >
        ...
        <motion.p
          variants={fadeUp}
          className="mt-7 max-w-xl text-base tablet:text-lg text-gray-500 dark:text-gray-400 leading-relaxed text-pretty"
        >
      ...
      <motion.div
        variants={fadeUp}
        className="relative w-44 tablet:w-52 laptop:w-60 aspect-square mx-auto tablet:mx-0 rounded-3xl overflow-hidden border border-gray-200 dark:border-white/[0.08] flex-shrink-0"
      >
        <Image
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 laptop:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-8 laptop:gap-14">
        ...
        >
          <p className="font-display text-xl tablet:text-2xl laptop:text-[1.75rem] font-medium text-gray-800 dark:text-gray-100 leading-[1.35] tracking-[-0.02em] text-balance">
        ...
        <div className="space-y-5">
          {paragraphs.map((paragraph, index) => (
            <motion.p
              ...
              className="text-base text-gray-600 dark:text-gray-400 leading-relaxed text-pretty"
            >
              {paragraph}
            </motion.p>
          ))}
    ...
    <section>
      ...
      <div className="grid grid-cols-1 laptop:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-6 laptop:gap-8">
        <div className="space-y-4">
          {education.map((item, index) => (
            <motion.article
              ...
              className="flex items-start gap-5 p-5 laptop:p-6 rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-dark-secondary"
            >
              <div className="relative w-16 h-16 flex-shrink-0 rounded-xl bg-white dark:bg-dark border border-gray-100 dark:border-white/[0.06]">
              ...
              <div className="min-w-0 flex-grow">
                ...
                <h3 className="font-display text-base laptop:text-lg font-bold text-gray-900 dark:text-white leading-snug tracking-[-0.015em]">
                  {item.degree}
                </h3>
              </div>
            </motion.article>
        ...
        <motion.div
          ...
          className="laptop:self-start p-6 rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-dark-secondary"
        >
          ...
          <dl className="space-y-4">
            {languages.map((item) => (
              <div
                key={item.name}
                className="flex items-baseline justify-between gap-4 pb-4 border-b border-gray-100 dark:border-white/[0.05] last:border-0 last:pb-0"
              >
                <dt className="font-display text-base font-bold text-gray-900 dark:text-white">
                  {item.name}
                </dt>
...
function ClosingCallToAction({ language, isEnglish }) {
  return (
    <motion.section
      ...
      className="pt-4"
    >
      <div className="h-px bg-gray-200 dark:bg-white/10 mb-10 tablet:mb-14" />

      <div className="flex flex-col tablet:flex-row tablet:items-end tablet:justify-between gap-8">
        <h2 className="font-display text-2xl tablet:text-3xl laptop:text-4xl font-bold text-gray-900 dark:text-white tracking-[-0.03em] leading-tight max-w-lg text-balance">
        ...
        <div className="flex flex-wrap gap-3 flex-shrink-0">
          ...
          >
            {isEnglish ? 'See my work' : 'Ver meus projetos'}
            <FiArrowUpRight className="w-4 h-4" />
          </Link>

          <Link
            href="/contact"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold border border-gray-300 dark:border-white/15 text-gray-800 dark:text-gray-200 hover:border-primary hover:text-primary dark:hover:text-primary transition-colors duration-200"
          >
